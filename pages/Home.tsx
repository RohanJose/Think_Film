...
const YouTubeEmbed: React.FC<{ videoId: string; active: boolean; shouldPlay: boolean }> = ({ videoId, active, shouldPlay }) => {
  ...
  const toggleMute = (e: React.MouseEvent) => {
    ...
    if (iframeRef.current) {
      const command = newMuteState ? 'unMute' : 'mute';
      iframeRef.current.contentWindow?.postMessage(
        JSON.stringify({ event: 'command', func: command, args: '' }),
        '*'
      );
  ...
  return (
    <div className="relative w-full h-full overflow-hidden bg-black group rounded-lg shadow-2xl">
      ...
      {/* Sound Toggle - Solid contrast for desktop visibility */}
      {active && shouldPlay && (
        <button
          onClick={toggleMute}
          className="absolute bottom-6 right-6 z-[100] flex items-center space-x-2 px-4 py-2 rounded-full bg-white text-black transition-all hover:scale-105 active:scale-95 shadow-2xl border border-black/5"
        >
  ...
  return (
    <main className="relative h-screen w-full bg-white">
      ...
      <div ref={containerRef} className="snap-container hide-scrollbar h-full w-full overflow-y-auto">
        ...
        {/* 5. ARCHIVE - Refined to overlay heading and prevent video cropping */}
        <section ref={(el) => { sectionRefs.current[5] = el; }} className="snap-section flex flex-col bg-white overflow-hidden relative">
          {/* Overlay Heading */}
          <div className="absolute top-24 md:top-32 left-8 md:left-12 z-[110]">
            <div className={`transition-all duration-1000 ${activeIndex === 5 ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-10'}`}>
              <h4 className="text-5xl md:text-[8rem] font-black uppercase tracking-tighter text-black/10 mix-blend-multiply pointer-events-none select-none">Archive.</h4>
            </div>
          </div>
          
          <div className="relative w-full h-full flex items-center justify-center px-4 md:px-8 max-w-[1600px] mx-auto z-10 pt-20">
            {/* Nav controls with high z-index */}
            <div className="absolute top-1/2 -translate-y-1/2 left-0 md:left-2 z-[120]">
               <button onClick={prevSlide} className="w-12 h-12 md:w-16 md:h-16 rounded-full border border-black/10 flex items-center justify-center bg-white/90 backdrop-blur-md shadow-2xl active:scale-90 transition-all hover:bg-black hover:text-white group">
            ...
            <div className="absolute top-1/2 -translate-y-1/2 right-0 md:right-2 z-[120]">
               <button onClick={nextSlide} className="w-12 h-12 md:w-16 md:h-16 rounded-full border border-black/10 flex items-center justify-center bg-white/90 backdrop-blur-md shadow-2xl active:scale-90 transition-all hover:bg-black hover:text-white group">
              ...
              >
                {showcaseVideos.map((id, idx) => (
                  <div key={id} className="w-full md:w-1/3 flex-shrink-0 px-4 md:px-8">
                    {/* RESTORED OLD SIZE - 9/16 Vertical Portrait */}
                    <div className="relative aspect-[9/16] group overflow-hidden bg-black rounded-lg transition-all duration-700 hover:scale-[1.01] shadow-2xl">
